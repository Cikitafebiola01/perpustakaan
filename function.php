...
{
	return mysqli_escape_string(konek(),strip_tags(trim($var))); // mencegah serangan sql
}
...
function kat(){
	...
	$kat = mysqli_query($GLOBALS['connect'],"SELECT * FROM kategori_buku ORDER By IDKategori ASC ");
...
function Bukurekomen(){
	...
	$Bukurekomen = mysqli_query($GLOBALS['connect'],"select * from buku order by RAND() LIMIT 0,4"); 
...
$sql =mysqli_query($GLOBALS['connect'],"SELECT a.*, b.namaKategori FROM buku a, kategori_buku b WHERE a.IDKategori=b.IDKategori AND b.IDKategori = '$IDKategori' order by RAND() LIMIT 0,12") 
